...
async function fetchDeviceByScan(qrData: string): Promise<Device> {
  const { data } = await api.get<Device>(`/devices/scan/${encodeURIComponent(qrData)}`);
  return data;
}
...
export function useDeviceByScan(qrData: string | null) {
  return useQuery<Device>({
    queryKey: ["deviceScan", qrData],
    queryFn: () => fetchDeviceByScan(qrData!),
    enabled: !!qrData,
    retry: false,
  });
}
